...
class TransferServiceSpec extends Specification {
	...
	def "test transfer"() {
		...
		timeService.isServiceAvailable(_ as LocalTime) >> { return true }
	...
	def "test illegal amount case"(){
		...
		then:
		thrown(expectedException)
		old(account1.balance) == account1.balance
		old(account2.balance) == account2.balance
	...
	@Unroll
	def "test transfer with time"() {
		setup:
		account1.setBalance(balance1)
		account2.setBalance(balance2)
		transferService.setFeePolicy(policy)

		_ * policy.getFee(_) >> {return fee}
		1 * timeService.isServiceAvailable(_ as LocalTime) >> {return serviceAvailable}
		...
		where:
		balance1	| balance2	| fee	| transferAmount 	| newBalance1 	| newBalance2	| serviceAvailable
		100			| 0			| 5		| 30				| 65			|	30			| true
		100			| 0			| 5		| 30				| 100			|	0			| false
	}
}
